...
public class GadgetScheduler {
	...
	public void onEnable() {
		...
		scheduleID = scheduler.scheduleSyncRepeatingTask(
				Mineageddon.getInstance(), new Gadget10TickSchedule(), 0, 10);
	}
			...
			for (Player player : Bukkit.getOnlinePlayers()) {
				if (Mineageddon.getConfigStaticly().isAllowedWorld(
					...
					if (Gadget.isVanillaItemGadget(vanillaID)) {
						Gadget.getGadgetFromVanillaID(vanillaID)
								.whilePlayerHoldingGadget(player, 10);
					}
