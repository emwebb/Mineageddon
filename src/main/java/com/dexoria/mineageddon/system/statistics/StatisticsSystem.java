					...
					public void run() {
						try {
							Mineageddon.getMySQL().updateSQL(
									"UPDATE mgStatistics SET diedWith = diedWith + 1 WHERE Gadget = '"
											+ gadgetName + "';");
		...
		Bukkit.getScheduler().runTaskAsynchronously(Mineageddon.getInstance(),
				new Runnable() {
					...
					public void run() {
						try {
							Mineageddon.getMySQL().updateSQL(
									"UPDATE mgStatistics SET killedWith = killedWith + 1 WHERE Gadget = '"
											+ gadgetName + "';");
						} catch (ClassNotFoundException | SQLException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
		...
		Bukkit.getScheduler().runTaskAsynchronously(Mineageddon.getInstance(),
				new Runnable() {
					...
					public void run() {
						try {
							Mineageddon.getMySQL().updateSQL(
									"UPDATE mgStatistics SET suicide = suicide + 1 WHERE Gadget = '"
											+ gadgetName + "';");
						} catch (ClassNotFoundException | SQLException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
	...
	public boolean gadgetExists(String gadget) {
		try {
			ResultSet res = Mineageddon.getMySQL().querySQL(
					"SELECT * FROM `mcph196778`.`mgStatistics` WHERE Gadget = '"
							+ gadget + "';");
